...
ysError dbasic::DeltaEngine::CreateGameWindow(const GameEngineSettings &settings) {
    ...
    ysWindowSystemObject::Platform platform;
    ysAudioSystem::API audioAPI;

#ifdef _MSC_VER
    platform = ysWindowSystemObject::Platform::Windows;
    audioAPI = ysAudioSystem::API::DirectSound8;
#else
    platform = ysWindowSystemObject::Platform::SDL;
    audioAPI = ysAudioSystem::API::SDL;
#endif

    // Create the window system
    YDS_NESTED_ERROR_CALL(ysWindowSystem::CreateWindowSystem(&m_windowSystem, platform));
    ...
    YDS_NESTED_ERROR_CALL(ysInputSystem::CreateInputSystem(&m_inputSystem, platform));
    ...
    YDS_NESTED_ERROR_CALL(ysAudioSystem::CreateAudioSystem(&m_audioSystem, audioAPI));
