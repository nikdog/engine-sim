...
ysError ysOpenGLDevice::CreateRenderingContext(ysRenderingContext **context, ysWindow *window) {
    ...
    *context = nullptr;
    ...
    if (window->GetPlatform() == ysWindowSystemObject::Platform::Windows) {
    ...
    else if (window->GetPlatform() == ysWindowSystemObject::Platform::SDL) {
        ...
        *context = static_cast<ysRenderingContext *>(newContext);
        ...
        return YDS_ERROR_RETURN_MSG(ysError::IncompatiblePlatforms, "Only Windows and SDL platforms are currently supported.");
    }
}
...
ysError ysOpenGLDevice::CreateVertexShader(ysShader **newShader, const char *shaderFilename, const char *shaderName) {
    ...
    *newShader = nullptr;
    ...
    strncpy(newOpenGLShader->m_shaderName, shaderName, 63);
    newOpenGLShader->m_shaderName[63] = '\0';
    strncpy(newOpenGLShader->m_filename, shaderFilename, 255);
    newOpenGLShader->m_filename[255] = '\0';
...
ysError ysOpenGLDevice::UseInputLayout(ysInputLayout *layout) {
    ...
    for (int i = 0; i < nChannels; i++) {
        ...
        m_realContext->glVertexAttribPointer(i, channel->m_length, channel->m_type, GL_FALSE, openglLayout->m_size, (void *)(intptr_t)channel->m_offset);
...
ysError ysOpenGLDevice::CreateTexture(ysTexture **texture, const char *fname) {
    ...
    *texture = nullptr;
    ...
    strncpy(newTexture->m_filename, fname, 256);
    newTexture->m_filename[256] = '\0';
    ...
    for (y = (newTexture->m_height - 1); y > -1; y--) {
        for (x = 0; x < newTexture->m_width; x++) {
            ...
            const SDL_PixelFormatDetails *fmt = SDL_GetPixelFormatDetails(pTexSurface->format);
            if (!useAlpha) {
                SDL_GetRGB(colour, fmt, NULL, &r, &g, &b);
            }
            else {
                SDL_GetRGBA(colour, fmt, NULL, &r, &g, &b, &a);
            }
    ...
    SDL_DestroySurface(pTexSurface);
    delete[] imageData;
...
ysError ysOpenGLDevice::CreateTexture(ysTexture **texture, int width, int height, const unsigned char *buffer) {
    ...
    *texture = nullptr;
    ...
    ysOpenGLTexture *newTexture = m_textures.NewGeneric<ysOpenGLTexture>();
    strncpy(newTexture->m_filename, "", 256);
    newTexture->m_filename[256] = '\0';
...
void ysOpenGLDevice::ResubmitInputLayout() {
    ...
    for (int i = 0; i < nChannels; i++) {
        ...
        if (channel->m_type == GL_FLOAT) {
            ...
            m_realContext->glVertexAttribIPointer(i, channel->m_length, channel->m_type, openglLayout->m_size, (void *)(intptr_t)channel->m_offset);
        }
...
void ysOpenGLDevice::Draw(int numFaces, int indexOffset, int vertexOffset) {
    if (m_activeVertexBuffer != nullptr) {
        m_realContext->glDrawElementsBaseVertex(GL_TRIANGLES, numFaces * 3, GL_UNSIGNED_SHORT, (void *)(intptr_t)(indexOffset * 2), vertexOffset);
    }
}
